...
function DriveImportModal({ allEntities, onClose, onDone, showToast }) {
  ...
  async function getAccessToken() {
    return new Promise((resolve, reject) => {
      ...
      // Timeout de 45s — detecta popup bloqueado pelo navegador
      const timer = setTimeout(() => {
        reject(new Error('O popup do Google não abriu. Permita popups para este site na barra do navegador e tente novamente.'));
      }, 45000);
    ...
    try {
      ...
      const parsed = files.map(f => {
        try {
          ...
          return { file: f, dirName: null, date: null, entity: null, selected: false };
        }
      });
      setRows(parsed);
      setStep('preview');
    ...
    <div className="modal-backdrop" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal" onClick={e => e.stopPropagation()} style={{ maxWidth: 520 }}>
        ...
        {(step === 'loading' || step === 'importing') && (
          <div style={{ padding: '40px 24px 48px', textAlign: 'center', color: 'var(--mist-dim)', fontSize: 13 }}>
            ...
            <div style={{ fontWeight: 500, color: 'var(--mist)', marginBottom: 8 }}>
              {step === 'loading' ? 'Aguardando autorização do Google…' : 'Importando vídeos…'}
            </div>
            {step === 'loading' && (
              <div style={{ fontSize: 12, color: 'var(--mist-muted)', lineHeight: 1.6, maxWidth: 300, margin: '0 auto' }}>
                Um popup do Google deve ter aberto. Se não aparecer, verifique se popups estão <strong style={{ color: 'var(--mist-dim)' }}>permitidos</strong> para este site na barra do navegador.
              </div>
            )}
          </div>
        )}
